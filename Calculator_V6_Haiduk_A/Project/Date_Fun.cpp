...
int ToYears(int &days) {
  int i = 0;
  bool isLeap = i % 4 == 0;
  while (days > 365 + isLeap) {  // counting years
...
int ToMonths(int &days) {
  ...
  bool isLeap = i % 4 == 0;
  bool unit = i % 2 != 0;
  bool isSecond = (i - 2) % 12 == 0;
  while(days > 30 + unit - (2 * isSecond) + isLeap) {
    days = days - (30 + unit - (2 * isSecond) + isLeap);
    ++i;
  }
  return i;
}
...
int CutAndDoDate(string expression) {
  ...
  for (int n = 0; n < 2; ++n) {
    for (int h = 0; h < 3; ++h) {
      if (ReadNumber(expression, num, i)) {
        int x;  // temporal variable for writing
        x = stoi(num);
        nums.push_back(x);
        num.clear();
      }
