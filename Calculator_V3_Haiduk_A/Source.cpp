...
double Factorial(double a) {
  if (a != round(a)) {
    cout << "ERROR: real number in factorial." << endl;
    return 0;
  }
  int res = 1;
  for (int i = 0; i < a; ++i) {
...
double CutAndDo(string expression) {
  vector<double> nums;      //array of operands;
  ...
  for (int i = 0; i < expression.length(); i) {
    while ((('0' <= expression[i]) && (expression[i] <= '9')) ||  //check for NUMBERS;
           expression[i] == '.') {
  ...
  if (nums.size() > 1) {
    ...
    for (int i = 0; i < operations.size() - 1; i++) {
      equal = ((operations[i] == '+') || (operations[i] == '-')) ==     //check for equal operations
              ((operations[i + 1] == '+') || (operations[i + 1] == '-'));  //in 3 or more operands expressions;
      if (equal == true || operations[i + 1] == '\0') {
        if (i+1 < nums.size())
          nums[i + 1] = Calculate(nums[i], operations[i], nums[i + 1]);                                                         
        else { 
          cout << "ERROR: the wrong format is used." << endl;   //exeptions a+-b, a*b**, sa-b, /a-b;
          return 0;
        }
      }
      else {
